...
public class RadarView extends View {
    ...
    //水滴的颜色
    private int mWaterColor = DEFAULT_COLOR;
    //水滴的数量 这里表示的是水滴最多能同时出现的数量。
    // 因为水滴是随机产生的，数量是不确定的
    private int mWaterNum = 4;
    //是否显示交叉线
    ...
    private Paint mWaterPaint;// 水滴的画笔
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            mWaterColor = mTypedArray.getColor(R.styleable.RadarView_raindropColor, DEFAULT_COLOR);
            mWaterNum = mTypedArray.getInt(R.styleable.RadarView_raindropNum, mWaterNum);
            isShowCross = mTypedArray.getBoolean(R.styleable.RadarView_showCross, true);
            isShowWater = mTypedArray.getBoolean(R.styleable.RadarView_showRaindrop, true);
    ...
    private void generateRaindrop(int cx, int cy, int radius) {
        ...
        if (mRaindrops.size() < mWaterNum) {
            ...
            if (probability) {
                ...
                mRaindrops.add(new Raindrop(x, y, 0, mWaterColor));
            }
        }
    }
    ...
    private void drawRaindrop(Canvas canvas, int cx, int cy, int radius) {
        ...
        for (Raindrop raindrop : mRaindrops) {
            mWaterPaint.setColor(raindrop.changeAlpha());
            canvas.drawCircle(raindrop.x, raindrop.y, raindrop.radius, mWaterPaint);
            //水滴的扩散和透明的渐变效果
