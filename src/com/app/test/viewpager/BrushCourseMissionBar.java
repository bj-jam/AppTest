...
public class BrushCourseMissionBar extends View {
    private final String infoString = "目标完成36%";
    ...
    public BrushCourseMissionBar(Context context) {
        super(context);
        init(null, 0);
    }

    public BrushCourseMissionBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public BrushCourseMissionBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        circleWidth = dip2px(getContext(), 5);
        textWidth = dip2px(getContext(), 15);
        infoWidth = dip2px(getContext(), 16);
        mTimeSize = dip2px(getContext(), 54);


        mColorPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mColorPaint.setColor(0xFF08E4B3);
        mColorPaint.setStyle(Style.STROKE);
        mColorPaint.setStrokeWidth(circleWidth);


        mDefaultPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mDefaultPaint.setColor(0xFF6B717A);
    ...
    private void drawThumbnail(Canvas canvas) {
        thumbX = (float) (mCircleX + getRealCosX(mCurrentAngle, mMaxRadius));
        thumbY = (float) (mCircleY + getRealSinY(mCurrentAngle, mMaxRadius));
