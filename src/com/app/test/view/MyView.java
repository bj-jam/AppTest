...
public class MyView extends View {
    ...
    public MyView(Context context) {
        ...
        this.paint_.setColor(Color.WHITE);
        this.smallCircleWidthPx_ = dipToPixels((int) SMALL_CIRCLE_MASK_WIDTH);
        this.cricleWdithPx_ = dipToPixels((int) CIRCLE_MASK_WIDTH);
        Log.d(TAG, "width=" + cricleWdithPx_ + " smallWidth="
                + smallCircleWidthPx_);
        this.centerPoint_ = new Point(this.cricleWdithPx_ / 2,
                this.cricleWdithPx_ / 2);
        this.radius = (this.cricleWdithPx_ / 2);
        this.smallRadius = (this.smallCircleWidthPx_ / 2);
        this.rectFOutside_ = new RectF(this.centerPoint_.x - this.radius,
                this.centerPoint_.y - this.radius, this.centerPoint_.x
                + this.radius, this.centerPoint_.y + this.radius);
        this.rectFInside_ = new RectF(this.centerPoint_.x - this.radius
                + this.smallCircleWidthPx_, this.centerPoint_.y - this.radius
                + this.smallCircleWidthPx_, this.centerPoint_.x + this.radius
                - this.smallCircleWidthPx_, this.centerPoint_.y + this.radius
                - this.smallCircleWidthPx_);
        this.rectFSmallUp_ = new RectF(this.centerPoint_.x - this.smallRadius,
                this.centerPoint_.y - this.radius, this.centerPoint_.x
                + this.smallRadius, this.centerPoint_.y - this.radius
                + this.smallCircleWidthPx_);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.currentRadian == 0) {
            ...
            canvas.drawCircle(this.centerPoint_.x, this.centerPoint_.y,
                    this.radius - this.smallRadius, this.paint_);
            return;
        }
        this.paint_.setStyle(Paint.Style.FILL);
        // acrto 其画曲线的顺序不能打乱的,其后面添加的是起点会和上次的结束点直线连接上.
        // 画一个外圆曲线,参数3为负数则逆时针画,正则顺时针画
        this.path_.arcTo(this.rectFOutside_, -90 + currentRadian,
                360 - currentRadian);
        // 连接上面的结束位置画一个小半圆
        this.path_.arcTo(this.rectFSmallUp_, -90.0F, -180.0F);
        // 从小半圆的结束位置画一个内圆曲线
        this.path_.arcTo(this.rectFInside_, -90, currentRadian - 360);
        float f1 = (float) (this.centerPoint_.x + Math.sin(Math
                .toRadians(this.currentRadian))
                * (this.radius - this.smallRadius));
        float f2 = (float) (this.centerPoint_.y - Math.cos(Math
                .toRadians(this.currentRadian))
                * (this.radius - this.smallRadius));
        this.rectFSmallDown_.set(f1 - this.smallRadius, f2 - this.smallRadius,
                f1 + this.smallRadius, f2 + this.smallRadius);
        // 接内圆曲线结束处画一个小半圆,连接到第一个开始出.
        this.path_
                .arcTo(this.rectFSmallDown_, 90 + this.currentRadian, -180.0F);
