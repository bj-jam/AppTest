...
public class CirclePieView extends View {
	...
	/** 圆形左边画图起点 */
	private float radiusX1 = 0;
	/** 圆形顶部画图起点 */
	private float radiusY1 = 0;
	/** 圆形矩形区域 */
	private RectF circleRectF;
	/** 圆形背景矩形区域 */
	private RectF circleRectFBG;
	/** 圆形背景矩形区域 */
	private RectF circleRectFBG1;
	/** 文字点坐标 */
	...
	private void init() {
		...
		// dis = res.getDimensionPixelSize(R.dimen.dp10);
		dis = 10;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (radius == 0) {
			...
			radiusX1 = (w - radius1 * 2) / 2;// 左边画图起点
			radiusY1 = (h - radius1 * 2) / 2;// 顶部画图起点
			// 画图矩形区域——左边起点，底部起点，左边起点+直径，顶部起点+直径
			circleRectFBG = new RectF(radiusX, radiusY, 2 * radius + radiusX, 2
					* radius + radiusY);

			circleRectFBG1 = new RectF(radiusX1, radiusY1, 2 * radius1
					+ radiusX1, 2 * radius1 + radiusY1);
		}
		if (sum > 0) {
			initCoords();
			drawPie(canvas);
	...
	private void initCoords() {
		...
		for (int i = 0; i < numbers.length; i++) {
			...
			// coords[1][0] = coords[1][0] + dis / 2;
			// coords[1][1] = coords[1][1] + dis / 2;
			// } else if (coords[0][2] == 1 && coords[2][2] == 1) {
			// coords[0][1] = coords[0][1] + dis;
			// } else if (coords[1][2] == 1 && coords[2][2] == 1) {
			// coords[1][1] = coords[1][1] + dis / 2;
			// }
			// }
		}
	...
	private void djfkadf() {
		...
		for (int i = 0; i < 5; i++) {
			...
			float x = new BigDecimal(Math.cos(goalAngle * Math.PI / 180))
					.setScale(1, BigDecimal.ROUND_HALF_UP).floatValue();
			inta[i][0] = radiusX1 + radius1 + radius1 * x;
			inta[i][1] = radiusY1 + radius1 + radius1 * y;
		}
	}
	...
	private void drawNoData(Canvas canvas) {
		...
		paint.setColor(Color.parseColor("#55D0CD"));
		// 画扇形——矩形区域，起始点，所占度数，填充扇形，画笔
		canvas.drawArc(circleRectFBG, 0, 360, true, paint);

		paint.setStyle(Style.STROKE);
		canvas.drawArc(circleRectFBG1, 0, 360, false, paint);
	...
	private void dfjad(Canvas canvas, int type) {
		paint.setStyle(Style.FILL);
		paint.setColor(Color.WHITE);
		paint.setStrokeWidth(1);
		canvas.drawCircle(inta[type][0], inta[type][1], 5, paint);
		invalidate();
	}
